...
BLOCK_SIZE = 16
BLOCKS = (MAX_INPUT.size / BLOCK_SIZE.to_f).ceil
PADDED_CIPHERTEXT_SIZE = BLOCK_SIZE * BLOCKS


padded_ciphertexts = CIPHERTEXTS.map do |c|
  size = c.size
  padded = c.dup.fill(nil, size, PADDED_CIPHERTEXT_SIZE - size)
  padded.each_slice(16).to_a
end

blocks_size = padded_ciphertexts.first.size
aligned_blocks = Array.new(blocks_size, [])

padded_ciphertexts.map do |cipher|
  blocks_size.times do |index|
    aligned_blocks[index] << cipher[index]
  end
end

keystream = aligned_blocks.flat_map do |aligned_block|
  aligned_block.transpose.map do |transposed_block|
    max_score = 0
    chosen_character = ''
...
CIPHERTEXTS.each do |c|
  puts xor_bytes_repeating(c, keystream.bytes.slice(0, c.size)).pack('C*')
end
