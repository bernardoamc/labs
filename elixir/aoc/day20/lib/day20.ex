defmodule Day20 do
  ...
  def part1(input), do: input |> final_map() |> Map.values() |> Enum.max()
  def part2(input), do: input |> final_map() |> Map.values() |> Stream.filter(&(&1 >= 1000)) |> Enum.count()

  defp final_map(input), do: walk(instructions(input)).map

  defp instructions(input), do: input |> Parser.ast()
  ...
  defmodule Parser do
    def ast(input), do: input |> tokens() |> build_ast()
    ...
    defp tokens(input), do: input |> String.trim() |> to_charlist() |> Enum.map(&token/1)
