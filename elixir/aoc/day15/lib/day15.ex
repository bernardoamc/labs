defmodule Day15 do
  ...
  def part1(), do: %{elf: 3, goblin: 3} |> play_game() |> Board.score()
  ...
  defp play_game(attack_powers) do
    [last_state] = attack_powers
      |> all_states()
      |> Enum.take(-1)
  ...
  defmodule Board do
    ...
    def next_play(board) do
      if board.units |> Map.values() |> Stream.map(& &1.type) |> all_same?() do
        :game_over
      else
    ...
    defp all_same?(types), do: length(Enum.uniq(types)) == 1
