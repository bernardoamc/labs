/*
 * The difference occurs because in the lseek version we don't
 * have atomicity, so a process A can sleep between the lseek and
 * write calls. If this happens and a process B open the same file
 * and execute a write to the end of it we will have an oudated
 * file offset in process A, now if we execute a write we will
 * override the previous bytes that were written by process B.
 */
